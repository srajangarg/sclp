...
void Symbol_Table::operator==(Symbol_Table &formal_list)
{
    ...
    for (i = variable_table.begin(), j = f_list.begin();
         i != variable_table.end() && j != f_list.end(); i++, j++) {
        Data_Type type1 = (*i)->get_data_type();
        Data_Type type2 = (*j)->get_data_type();

        CHECK_INPUT((type1 == type2), "Return type of one of the parameters of the "
                                      "procedure and its prototype doesn't match",
                    NO_FILE_LINE);

        string name1 = (*i)->get_variable_name();
        string name2 = (*j)->get_variable_name();

        CHECK_INPUT((name1 == name2), "Variable name of one of the parameters of the "
                                      "procedre and its prototypes doesn't match",
                    NO_FILE_LINE);
    }

    CHECK_INPUT((i == variable_table.end() && j == f_list.end()),
                "Procedure and its prototype parameter f_list length doens't match",
                NO_FILE_LINE);
}
