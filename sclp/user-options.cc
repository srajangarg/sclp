...
bool User_Options::is_show_ic_selected()
{
    return show_ic_selected;
}
bool User_Options::is_do_compile_selected()
{
    return do_compile_selected;
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strncmp(option, "-", 1)) {
            if (!strcmp(option, "-d"))
                ...
                CHECK_INPUT_AND_ABORT(CONTROL_SHOULD_NOT_REACH, error_message,
                                      NO_FILE_LINE);
            }
    ...
    if (demo_mode_selected == false) {
        ...
        if (show_tokens_selected && !tokens_file_buffer)
            CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
                            "Unable to open output file for tokens");

        if (show_ast_selected && !ast_file_buffer)
            CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
                            "Unable to open output file for AST");

        if (show_symtab_selected && !symtab_file_buffer)
            CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
                            "Unable to open output file for symtab");

        if (show_ic_selected && !ic_file_buffer)
            CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
                            "Unable to open output file for icode");
    }
