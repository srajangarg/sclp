    ...
    switch (dt) {
        ...
        case void_data_type:
            CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
                            "Attempt to seek size of type void");
        default:
...
void Symbol_Table::print(ostream &file_buffer)
{
    ...
    for (i = variable_table.begin(); i != variable_table.end(); i++) {
        ...
        switch (dt) {
            case int_data_type:
                file_buffer << " Type: INT";
                break;
            defualt:
                CHECK_INVARIANT(CONTROL_SHOULD_NOT_REACH,
                                "Variable data type can only be int");
        }
...
{
    CHECK_INVARIANT((destination_reg_descr != NULL), "The register descriptor of the "
                                                     "destination should not be NULL "
                                                     "while freeing a register");
...
{
    CHECK_INVARIANT((result_reg_descr != NULL), "The register descriptor of the result "
                                                "should not be NULL while updating "
